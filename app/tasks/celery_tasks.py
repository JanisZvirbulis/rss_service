...
def collect_all_rss_feeds():
    ...
    try:
        ...
        results = {"success": 0, "error": 0, "new_entries": 0}
        ...
        for feed in active_feeds:
            try:
                ...
                feed_fresh = (
                    feed_db.query(RssFeed).filter(RssFeed.id == feed.id).first()
                )
        ...
        logger.info(
            f"RSS ievākšana pabeigta. Veiksmīgi: {results['success']}, "
...
def collect_single_rss_feed(feed_id: int):
    ...
    try:
        ...
        if success:
            logger.info(
                f"Veiksmīgi ievākti dati no barotnes {feed.url}. Pievienoti {entry_count} jauni ieraksti."
            )
        ...
        for entry_data in feed_data.entries:
            ...
            if not existing_entry:
                ...
                # Palaižam pilnā raksta iegūšanu asinhronā režīmā
                fetch_full_article_content.delay(new_entry.id)
                new_entries += 1

        return {"status": "success", "new_entries": new_entries}
...
def get_clean_article_text(url):
    try:
        ...
        response.raise_for_status()  # Pārbaudam, vai nav HTTP kļūdu

        # Izmantojam readability, lai iegūtu raksta galveno daļu
        doc = Document(response.text)
        article_html = doc.summary()

        # Izmantojam BeautifulSoup, lai noņemtu visus HTML tagus
        soup = BeautifulSoup(article_html, "html.parser")

        # Noņemam "nevēlamos" elementus (reklāmas, komentāri, utt.)
        for unwanted in soup.find_all(
            ["script", "style", "iframe", "noscript", "aside"]
        ):
            unwanted.decompose()

        # Noņemam arī klases vai ID, kas varētu būt saistīti ar reklāmām vai navigāciju
        for unwanted in soup.find_all(
            class_=lambda x: x and ("ads" in x or "piano" in x or "sidebar" in x)
        ):
            unwanted.decompose()

        # Iegūstam tīru tekstu
        clean_text = soup.get_text(separator="\n\n")

        # Papildu teksta attīrīšana
        clean_text = "\n".join(
        ...
        return f"Kļūda iegūstot rakstu: {str(e)}"


@shared_task(name="fetch_full_article_content")
def fetch_full_article_content(entry_id: str):
    """
    Celery uzdevums, kas iegūst pilno rakstu no norādītās saites
    """
    db = SessionLocal()
    try:
        logger.info(f"Sākas pilnā raksta ieguve!")
        entry = db.query(Entry).filter(Entry.id == entry_id).first()
        if not entry:
            logger.error(f"Ieraksts nav atrasts!")
            return {"status": "error", "message": "Ieraksts nav atrasts"}

        # Iegūstam tīru raksta tekstu
        ...
        if clean_text and not clean_text.startswith("Kļūda"):
            ...
            # Šeit varētu izsaukt kopsavilkuma veidošanu
            # create_article_summary.delay(entry_id)
            logger.info(f"Raksts veiksmīgi iegūts!")
            return {"status": "success", "message": "Raksts veiksmīgi iegūts"}
        else:
            logger.error(f"Neizdevās iegūt rakstu: {clean_text}")
            return {
                "status": "error",
                "message": f"Neizdevās iegūt rakstu: {clean_text}",
            }
    except Exception as e:
        logger.error(f"Kļūda: {str(e)}")
        return {"status": "error", "message": f"Kļūda: {str(e)}"}
    finally:
        db.close()
