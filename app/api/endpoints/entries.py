...
def read_entries(
    ...
    query = db.query(Entry, RssFeed.title.label("feed_title"))\
        .join(RssFeed, Entry.feed_id == RssFeed.id)
...
def delete_all_entries(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Kļūda dzēšot ierakstus: {str(e)}"
        )
...
def delete_all_tags(
    ...
    try:
        ...
        unused_tags = db.query(Tag.id).outerjoin(
            entry_tag, Tag.id == entry_tag.c.tag_id
        ).filter(entry_tag.c.entry_id == None).subquery()
        
        # Dzēšam neizmantotos tagus
        tags_count = db.query(Tag).filter(Tag.id.in_(unused_tags)).delete(synchronize_session=False)
        db.commit()
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Kļūda dzēšot tagus: {str(e)}"
        )
...
def delete_all_tags_with_relations(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Kļūda dzēšot tagus un saites: {str(e)}"
        )