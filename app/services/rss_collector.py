...
class RssCollector:
    ...
    def fetch_all_feeds(self) -> Dict[str, int]:
        ...
        with concurrent.futures.ThreadPoolExecutor(
            max_workers=settings.RSS_CONCURRENT_REQUESTS
        ) as executor:
            future_to_feed_id = {
            ...
            for future in concurrent.futures.as_completed(future_to_feed_id):
                ...
                try:
                    ...
                    logger.error(
                        f"Barotnes ar ID {feed_id} apstrāde izraisīja izņēmumu: {exc}"
                    )
                    results["error"] += 1
    ...
    def fetch_single_feed(self, feed: RssFeed) -> tuple[bool, int]:
        ...
        try:
            ...
            if parsed_feed.bozo and hasattr(parsed_feed, "bozo_exception"):
                ...
                logger.warning(
                    f"RSS barotnē {feed.url} ir kļūdas: {parsed_feed.bozo_exception}"
                )
            ...
            self.db.commit()
            logger.info(
                f"Barotnei {feed.url} pievienoti {new_entries_count} jauni ieraksti"
            )
            ...
            if feed.error_count >= 5:  # Pēc 5 secīgām kļūdām deaktivizējam
                logger.warning(
                    f"Barotne {feed.url} deaktivizēta pēc {feed.error_count} secīgām kļūdām"
                )
                feed.active = False
    ...
    def _prepare_metadata(self, entry) -> Dict[str, Any]:
        ...
        if "geo_lat" in entry and "geo_long" in entry:
            metadata["geo"] = {"lat": entry.geo_lat, "long": entry.geo_long}
